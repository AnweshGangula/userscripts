...
(function () {
    ...
    /** @type {Object} Configuration for UI styling */
     ...
     */
    class UIManager {
        constructor(securityService, logoSvg) {
            this.security = securityService;
            this.logo = logoSvg;
            this.selectors = {
                sidebar: "#rhs",
                container: "OmnisearchObsidianResults",
                loading: "OmnisearchObsidianLoading"
            };
        }
        ...
        injectContainer() {
            ...
            const html = `
                <details id="ObsidianSearchDetailsS" open>
                    ...
                    <div id="${this.selectors.container}" style="margin-top: var(--omni-spacing); display: flex; flex-direction: column; gap: var(--omni-spacing)"></div>
                </details>`;
            $(this.selectors.sidebar).prepend(html);
        }
        ...
        renderHeader(onSettingsClick) {
            const header = $(`
                ...
                <span style="font-size: 12px">(<a id="OmnisearchSettingsTrigger" class="feedback-link-btn" title="Settings" href="#">settings</a>)</span>
            `);
            $('#OmnisearchHeader').append(header);
            $(document).on("click", "#OmnisearchSettingsTrigger", (e) => {
                e.preventDefault();
                onSettingsClick();
            });
        }
        ...
        setLoading(message = "Loading...") {
            $(`#${this.selectors.container}`).html(`<span id="${this.selectors.loading}">${message}</span>`);
        }
    ...
    class OmnisearchApp {
        constructor() {
            ...
            this.styler = new StyleManager(STYLING_CONFIG);
            this.security = new SecurityService();
            this.api = new OmnisearchService(this.config);
            this.ui = new UIManager(this.security, this.logo);
        }
        ...
        _waitForConfig() {
            return new Promise((resolve) => {
                let check = () => this.config.isInit ? resolve() : setTimeout(check, 50);
                check();
            });
        }
        ...
        async runSearch(query) {
            ...
            try {
                ...
                const results = this.api.processResults(rawData);

                const container = $(`#${this.ui.selectors.container}`).empty();
                if (results.length === 0) {
                    container.html('<span style="color: #70757a;">No results found</span>');
                } else {
